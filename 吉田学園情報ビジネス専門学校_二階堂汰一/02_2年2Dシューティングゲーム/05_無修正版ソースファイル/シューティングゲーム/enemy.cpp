...
CEnemy::CEnemy(int nPriority):CScene2d(nPriority)
{
	...
	m_nBulletIndex = 0;							//発射数
	m_fBulletAngle = 0.0f;						//弾の角度
	m_fWidth = 0.0f;							//幅
	m_fHeight = 0.0f;							//高さ
	m_type = TYPE_NONE;							//敵の種類
	memset(m_Location, 0, sizeof(m_Location));	//場所
	m_fTime = 0.0f;								//タイム
	m_fStop = 0.0f;								//ストップ
	m_nEnemyTimeCount = 0;						//敵が存在している時間
	...
	m_bShot = false;							//球を撃ったかどうか
	m_bStay = true;								//滞在してるか
	m_nColorCount = 0;								//色のカウント
	m_fRd = 0.0f;								//ラジアン
	m_fRdScale = 0.1f;							//スケール
}
...
HRESULT CEnemy::Init(D3DXVECTOR3 pos, float SizeWidth, float SizeHeight,  TYPE type,int nLife)
{
	...
	m_fTime = 0.0f;							//時間
	m_nEnemyTimeCount = 0;					//敵の滞在時間
	m_nHitCount = 0;						//HITカウント
	m_nCounterAnim = 0;						//アニメカウンタ
	m_nPatternAnim = 0;						//パターンアニメ
	// 対角線、角度の設定
	m_fLength = sqrtf((float)(((m_fWidth / 2) * (m_fWidth / 2)) + ((m_fHeight / 2) * (m_fHeight / 2))));
	m_fAngle = atan2f((m_fHeight / 2), (m_fWidth / 2));	//角度
	m_fRot = 0.0f;							//向き
	//もしタイプがボムヘッドとボムボディだった時
	if (m_type == TYPE_BOMBHEAD||m_type == TYPE_BOMBBODY)
	{
		//向き
		m_fRot = D3DXToRadian(90.0f) ;
	}
	m_fScale = 1.0f;	//拡縮
	m_bHit = false;		//ヒットしたかどうか
	m_bShot = false;	//弾撃ったかどうか
	m_bStay = true;		//滞在してるかどうか
	m_nStayCount = 0;	//滞在時間
	m_nColorCount = 0;		//色のカウント
	//ランダムシード値
	...
	//頂点座標の設定
	m_vpos[0].x = m_pos.x - cosf(m_fAngle - m_fRot) * m_fLength * m_fScale;
	m_vpos[0].y = m_pos.y - sinf(m_fAngle - m_fRot) * m_fLength * m_fScale;
	m_vpos[0].z = 0.0f;

	m_vpos[1].x = m_pos.x + cosf(m_fAngle + m_fRot) * m_fLength * m_fScale;
	m_vpos[1].y = m_pos.y - sinf(m_fAngle + m_fRot) * m_fLength * m_fScale;
	m_vpos[1].z = 0.0f;

	m_vpos[2].x = m_pos.x - cosf(m_fAngle + m_fRot) * m_fLength * m_fScale;
	m_vpos[2].y = m_pos.y + sinf(m_fAngle + m_fRot) * m_fLength * m_fScale;
	m_vpos[2].z = 0.0f;

	m_vpos[3].x = m_pos.x + cosf(m_fAngle - m_fRot) * m_fLength * m_fScale;
	m_vpos[3].y = m_pos.y + sinf(m_fAngle - m_fRot) * m_fLength * m_fScale;
	m_vpos[3].z = 0.0f;
	//頂点座標のセット
...
void CEnemy::AI(void)
{
			...
			if (pScene != NULL)
			{
				...
				if (objType == OBJTYPE_PLAYER)
				{
					if (m_bShot == false)
					{
						...
						m_fRot = atan2f(m_TargetDistance.y, m_TargetDistance.x);//プレイヤの方に向けた角度を計算
						m_fRot = m_fRot * -1.0f;
					}
				}
					...
					if (CPlayer::GetbDeath() == false)
					{
						if (m_nCountBullet % 20 == 0)
						{
							if (m_nColorCount < 7)
							{
								...
								{
									CBullet::Create(m_pos, D3DXVECTOR3(cosf(D3DXToRadian(nCnt * (360 / 20)))*3.5f, sinf(D3DXToRadian(nCnt * (360 / 20)))*3.5f, 0.0f), BULLET_SIZE * 4, BULLET_SIZE * 4, CBullet::USER_ENEMY, CBullet::TYPE_FLOWER, (CBullet::COLOR)m_nColorCount, CBullet::FLOWER_TEXTURE, EASY_ENEMY_BULLET_ATTACK);
								}
							}
	...
	case TYPE_BOMBHEAD:
		...
		//進行方向に向きを合わせる
		m_fRot = atan2f((m_pos.x + m_move.x) - m_pos.x, (m_pos.y + m_move.y) - m_pos.y);
		break;
	case TYPE_BOMBBODY:
		m_fRd += m_fRdScale;
		if (m_fRd >= 360.0f)
