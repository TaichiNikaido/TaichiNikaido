...
// マクロ定義
//*****************************************************************************
#define TEXTURE ("Data/Texture/gameclear_logo.png")		//テクスチャのパス
#define SIZE (D3DXVECTOR3(1000.0f,300.0f,0.0f))			//サイズ
#define INIT_COUNT (0)									//カウントの初期値
#define INIT_TIME (0)									//時間の初期値
#define CLEAR_TIMNE (200)								//クリア表示時間
#define SHOW_COLOR (D3DXCOLOR(1.0f,1.0f,0.0f,1.0f))		//見得ていない場合の色
#define HIDE_COLOR (D3DXCOLOR(0.0f,1.0f,1.0f,0.0f))		//見えていない場合の色
...
void CGameClearLogo::Update()
{
	...
	//弾～敵のプライオリティ分回す
	for (int nCountPriority = 0; nCountPriority <= PRIORITY_ENEMY; nCountPriority++)
	{
		//シーンの総数分回す
		for (int nCountScene = 0; nCountScene < GetNumAll(); nCountScene++)
		{
			...
			if (pScene != NULL)
			{
				...
				//もしオブジェクトタイプが火球だったら
				if (ObjType == OBJECT_TYPE_FIRE_BALL)
				{
					//火球のポインタ取得
